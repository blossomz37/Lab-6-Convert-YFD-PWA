...
1. **Standardizing chapter headings**: Ensures consistent H2 (`##`) format for all chapters
2. **Converting to DOCX format**: Converts markdown to Word document format for ProWriting Aid compatibility
...
3. **Output**: Converted files will be created in the `./docx_pwa/` directory
...
- Standardizes chapter heading format to `## Chapter N:` (ensures colons after numbers)
- Uses pandoc to convert markdown to DOCX format
- Creates individual files in `./docx_pwa/` directory
- Combines all chapters into `all_chapters.docx` with page breaks
...
## Chapter 1: Dark Alchemy
```
*Note: This ensures consistent formatting and gets converted to DOCX with proper H2 headings*
...
- Combined manuscript: `all_chapters.docx` (all 20 chapters with page breaks)
- Standardized H2 chapter headings with consistent colon format
...
- The script creates both individual chapter files and a combined document
